...
*Pyricularia oryzae*, now known as *Magnaporthe oryzae* (2002), causes rice blast and is a major plant pathogen infecting rice and other cereal crops such as wheat, rye, and barley. Sequencing this fungus provides valuable insights into the pathogen’s host range, evolution, and potential for cross-infection. This genome contributes to global surveillance efforts to support comparative and resistance-focused studies essential for managing blast disease. *P. oryzae* sample was collected from host plant *Paspalum distichum* in Nueva Ecija, Philippines (1988) and sequenced with the ILLUMINA NovaSeq X platform. The resulting genome assembly was submitted to NCBI (BioSample ID: SAMN46817650; SRA accession #: SRR32338628; BioProject: PRJNA926786).
...
*Image above is from the [reverse reads](data/FastQC/Pd8825_2_fastqc.html).*
</details>
...
```
To run the command above, the [trimmomatic-0.38.jar](files/trimmomatic-0.38.jar) and [adaptors.fasta](files/adaptors.fasta) files are required. 
...
*Image above is from the [reverse reads](data/FastQC/Pd8825_2_trimmed_fastqc.html).*
</details>
...
| ------------- | ------------- | -------------- |
| [Raw Forward](data/FastQC/Pd8825_1_fastqc.html)  | 7692782  | ~12% |
| [Raw Reverse](data/FastQC/Pd8825_2_fastqc.html)  | 7692782  | ~12% |
| [Trimmed Forward](data/FastQC/Pd8825_1_trimmed_fastqc.html)  | 6356740  | ~2% |
| [Trimmed Reverse](data/FastQC/Pd8825_2_trimmed_fastqc.html)  | 6356740  | ~2% |

***NOTE:** HTML files from the FastQC before and after trimming can be found under the [FastQC](data/FastQC) directory and are linked in the table above.*
## Assembly
...
3. Run VelvetOptimiser by steps of 10 to find an approximate optimal k-mer value. 
***NOTE:** I used a bash script to run this command. This bash file can be found under [`SLURM_SCRITPS`](SLURM_SCRIPTS/velvetoptimiser_noclean.sh).*
```
...
```
***NOTE:** Scripts can be found under [`SCRIPTS`](SCRIPTS)*
...
```
***NOTE:** Scripts can be found under [`SLURM_SCRIPTS`](SLURM_SCRIPTS/BuscoSingularity.sh)*
Results from BUSCO:
...
```
***NOTE:** Bash script can be found under [`SLURM_SCRIPTS`](SLURM_SCRIPTS/trim-spades.sh)*
...
```
***NOTE:** [MoMitochondrion.fasta](files/MoMitochondrion.fasta) is required for the command above and the resulting csv file can be found in the [data](data/Pd8825_mitochondrion.csv) directory. Additionally, the following command is required before running the code above if blastn is not installed in machine.*
```
singularity run --app blast2120 /share/singularity/images/ccs/conda/amd-conda1-centos8.sinf
```
